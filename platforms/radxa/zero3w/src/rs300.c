...
#define RS300_PIXEL_RATE	(40 * 1000 * 1000)   /* 40MHz pixel rate matching Rockchip reference */
...
#define V4L2_CID_RS300_COLORMAP		(V4L2_CID_CUSTOM_BASE + 1)
#define V4L2_CID_RS300_FFC		(V4L2_CID_CUSTOM_BASE + 2)
#define V4L2_CID_RS300_SCENE_MODE	(V4L2_CID_CUSTOM_BASE + 3)
#define V4L2_CID_RS300_DDE		(V4L2_CID_CUSTOM_BASE + 4)
#define V4L2_CID_RS300_SPATIAL_NR	(V4L2_CID_CUSTOM_BASE + 5)
#define V4L2_CID_RS300_TEMPORAL_NR	(V4L2_CID_CUSTOM_BASE + 6)
#define V4L2_CID_RS300_OUTPUT_MODE	(V4L2_CID_CUSTOM_BASE + 7)
#define V4L2_CID_RS300_AUTOSHUTTER	(V4L2_CID_CUSTOM_BASE + 8)
#define V4L2_CID_RS300_AUTOSHUTTER_TEMP	(V4L2_CID_CUSTOM_BASE + 9)
#define V4L2_CID_RS300_AUTOSHUTTER_MIN	(V4L2_CID_CUSTOM_BASE + 10)
#define V4L2_CID_RS300_AUTOSHUTTER_MAX	(V4L2_CID_CUSTOM_BASE + 11)
#define V4L2_CID_RS300_CAMERA_SLEEP	(V4L2_CID_CUSTOM_BASE + 12)
#define V4L2_CID_RS300_ANTIBURN		(V4L2_CID_CUSTOM_BASE + 13)
#define V4L2_CID_RS300_SHUTTER		(V4L2_CID_CUSTOM_BASE + 14)
#define V4L2_CID_RS300_HOOK_EDGE	(V4L2_CID_CUSTOM_BASE + 15)
#define V4L2_CID_RS300_FRAME_RATE	(V4L2_CID_CUSTOM_BASE + 16)
#define V4L2_CID_RS300_ANALOG_OUTPUT	(V4L2_CID_CUSTOM_BASE + 17)
#define V4L2_CID_RS300_YUV_FORMAT	(V4L2_CID_CUSTOM_BASE + 18)
...
/* Define YUV byte order menu items */
...
MODULE_PARM_DESC(mode, "Sensor resolution (0=640x512, 1=256x192, 2=384x288)");
module_param(fps, int, 0644);
MODULE_PARM_DESC(fps, "Target frame rate (default: 60)");
module_param(type, int, 0644);
MODULE_PARM_DESC(type, "Pixel bit depth (default: 16)");
...
static const u32 codes[] = {
	MEDIA_BUS_FMT_YUYV8_2X8,   /* YUV 4:2:2 dual-lane - camera outputs YUYV */
};
...
struct rs300 {
	...
	struct v4l2_ctrl *yuv_format;  /* YUV byte order control */
	...
	/* Rockchip module identification */
	u32 module_index;
...
static int __maybe_unused rs300_get_autoshutter(struct rs300 *rs300, int *value);
...
static struct rs300_mode supported_modes[] = {
    { /* 640 - Primary mode for Pi 5 */
        ...
        .code = MEDIA_BUS_FMT_YUYV8_2X8, /* YUV 4:2:2 dual-lane - camera outputs YUYV */
    },
    {
        .width      = 256,
        .height     = 192,
				 ...
				 struct rkmodule_inf *inf)
{
	...
	strscpy(inf->base.module, rs300->module_name ? rs300->module_name : "",
		sizeof(inf->base.module));
	strscpy(inf->base.lens, rs300->len_name ? rs300->len_name : "",
...
static long rs300_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	/* Handle Rockchip module info ioctl before userspace pointer validation */
	if (cmd == RKMODULE_GET_MODULE_INFO) {
		rs300_get_module_inf(to_rs300(sd), (struct rkmodule_inf *)arg);
		return 0;
	}
...
{
	/* YUYV8_2X8 YUV 4:2:2 colorspace */
...
static int rs300_set_output_mode(struct rs300 *rs300, int value)
{
    ...
    /* Validate value range (0=YUYV, 1=Y16) */
    if (value < 0 || value > 1) {
        dev_err(&client->dev, "Invalid output mode value: %d (valid: 0-1)", value);
        return -EINVAL;
    }
    ...
    /* Pre-calculated CRC values for each output mode */
    static const u8 mode_crc[2][2] = {
        {0xFB, 0xC0},  /* Mode 0: YUYV output */
        {0x8E, 0xC3},  /* Mode 1: Y16 output */
    };
...
static int rs300_set_yuv_format(struct rs300 *rs300, int format)
{
    ...
    cmd_buffer[3] = format; /* Parameter: 0=UYVY, 1=VYUY, 2=YUYV, 3=YVYU */
    cmd_buffer[4] = 0x00;  /* Reserved */
...
static int __maybe_unused rs300_get_sleep(struct rs300 *rs300, int *value)
{
    struct i2c_client *client = v4l2_get_subdevdata(&rs300->sd);
    u8 cmd_buffer[18];
    u8 status_buffer[1];
...
static int rs300_set_ctrl(struct v4l2_ctrl *ctrl)
{
    ...
    case V4L2_CID_ZOOM_ABSOLUTE:
        ...
        break;
    case V4L2_CID_RS300_SCENE_MODE:
    ...
    case V4L2_CID_CONTRAST:
        ...
        break;
    case V4L2_CID_RS300_DDE:  /* DDE */
        ret = rs300_set_dde(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_SPATIAL_NR:  /* Spatial NR */
        ret = rs300_set_spatial_nr(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_TEMPORAL_NR:  /* Temporal NR */
        ret = rs300_set_temporal_nr(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_OUTPUT_MODE:  /* Output Mode */
        ret = rs300_set_output_mode(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_AUTOSHUTTER:  /* Autoshutter enable/disable */
        ret = rs300_set_autoshutter(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_AUTOSHUTTER_TEMP:  /* Autoshutter temperature threshold */
        ret = rs300_set_autoshutter_params(rs300, 0, ctrl->val);
        break;
    case V4L2_CID_RS300_AUTOSHUTTER_MIN:  /* Autoshutter min interval */
        ret = rs300_set_autoshutter_params(rs300, 1, ctrl->val);
        break;
    case V4L2_CID_RS300_AUTOSHUTTER_MAX:  /* Autoshutter max interval */
        ret = rs300_set_autoshutter_params(rs300, 2, ctrl->val);
        break;
    case V4L2_CID_RS300_CAMERA_SLEEP:  /* Camera sleep */
        dev_dbg(&client->dev, "Setting camera sleep: %s", ctrl->val ? "ON" : "OFF");
        ret = rs300_set_sleep(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_ANTIBURN:  /* Anti-burn Protection */
        ret = rs300_set_antiburn(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_SHUTTER:  /* Shutter State */
        ret = rs300_set_shutter(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_HOOK_EDGE:  /* Hook Edge Position */
        ret = rs300_set_hook_edge(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_FRAME_RATE:  /* Detector Frame Rate */
        ret = rs300_set_frame_rate(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_ANALOG_OUTPUT:  /* Digital-Analog Output Format */
        ret = rs300_set_analog_output_fmt(rs300, ctrl->val);
        break;
    case V4L2_CID_RS300_YUV_FORMAT:  /* YUV Byte Order */
				 ...
				 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->pad == IMAGE_PAD) {
		if (code->index >= ARRAY_SIZE(codes)) {
			dev_err(&client->dev, "Invalid format code index %d (max %lu)",
				code->index, ARRAY_SIZE(codes)-1);
			return -EINVAL;
		}
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->pad == IMAGE_PAD) {
		...
		rs300_update_image_pad_format(rs300, mode, fmt);
			...
			dev_dbg(&client->dev, "Set ACTIVE format: code=0x%x, %dx%d",
...
static int rs300_set_framefmt(struct rs300 *rs300)
{
    ...
    case MEDIA_BUS_FMT_YUYV8_2X8:
        dev_dbg(&client->dev, "Using YUYV8_2X8 format");
        return 0;
    case MEDIA_BUS_FMT_UYVY8_2X8:
        dev_dbg(&client->dev, "Using UYVY8_2X8 format");
        return 0;
    case MEDIA_BUS_FMT_YUYV8_1X16:
        dev_dbg(&client->dev, "Using YUYV8_1X16 format");
        return 0;
    case MEDIA_BUS_FMT_UYVY8_1X16:
        dev_dbg(&client->dev, "Using UYVY8_1X16 format");
        return 0;
    default:
...
static int rs300_set_stream(struct v4l2_subdev *sd, int enable)
{
    ...
    if (enable) {
        ...
        dev_err(&client->dev, "Format: 0x%x (%s), Resolution: %dx%d", 
            ...
            rs300->fmt.code == MEDIA_BUS_FMT_YUYV8_2X8 ? "YUYV8_2X8" :
            rs300->fmt.code == MEDIA_BUS_FMT_UYVY8_2X8 ? "UYVY8_2X8" : "OTHER",
    ...
    if (enable) {
        ...
        dev_dbg(&client->dev, "Stream started successfully");
        ...
        for (stream_attempt = 0; stream_attempt < STREAM_START_RETRIES; stream_attempt++) {
            ...
            dev_dbg(&client->dev, "Attempt %d/%d - Final stream status: 0x%02x",
            ...
            if (status_buffer[0] & VCMD_ERR_STS_BIT) {
                ...
                if (stream_attempt < STREAM_START_RETRIES - 1) {
                    ...
                    dev_dbg(&client->dev, "Re-sending start command for retry %d", stream_attempt + 2);
                    if (write_regs(client, I2C_VD_BUFFER_RW, start_regs, sizeof(start_regs)) < 0) {
        ...
        dev_dbg(&client->dev, "Stream started successfully, streaming flag set");
    } else {
...
error_unlock:
    ...
    if (enable) {
        dev_dbg(&client->dev, "Sending STOP command to clean up after failed START");
        rs300_stop_streaming(rs300);
    }
    mutex_unlock(&rs300->mutex);
    return ret;
			  ...
			  struct v4l2_subdev_state *state)
{
	...
	/* Write default format to the TRY slot */
	format = v4l2_subdev_get_try_format(sd, state, IMAGE_PAD);
				  ...
				  struct v4l2_subdev_frame_interval *fi)
{
	...
	fi->interval.denominator = rs300->mode->max_fps.numerator;
	mutex_unlock(&rs300->mutex);
...
static int rs300_get_mbus_config(struct v4l2_subdev *sd,
				 unsigned int pad,
...
static const struct v4l2_subdev_pad_ops rs300_subdev_pad_ops = {
	...
	.get_mbus_config = rs300_get_mbus_config,
};
...
static int rs300_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct rs300 *rs300 = to_rs300(sd);
...
static const struct v4l2_ctrl_config autoshutter_temp_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_AUTOSHUTTER_TEMP,
...
static const struct v4l2_ctrl_config autoshutter_min_interval_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_AUTOSHUTTER_MIN,
...
static const struct v4l2_ctrl_config autoshutter_max_interval_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_AUTOSHUTTER_MAX,
...
static const struct v4l2_ctrl_config camera_sleep_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_CAMERA_SLEEP,
    .name = "Camera Sleep",
    .type = V4L2_CTRL_TYPE_BOOLEAN,
    .min = 0,
    .max = 1,
...
static const struct v4l2_ctrl_config analog_output_fmt_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_ANALOG_OUTPUT,
...
static const struct v4l2_ctrl_config output_mode_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_OUTPUT_MODE,
    .name = "Output Mode",
    .type = V4L2_CTRL_TYPE_MENU,
    .qmenu = output_mode_menu,
    .min = 0,
    .max = 1,
    .def = 0,  /* Default to YUYV output */
};

static const struct v4l2_ctrl_config yuv_format_ctrl = {
    .ops = &rs300_ctrl_ops,
    .id = V4L2_CID_RS300_YUV_FORMAT,
    .name = "YUV Byte Order",
    .type = V4L2_CTRL_TYPE_MENU,
    .qmenu = yuv_format_menu,
    .min = 0,
    .max = 3,
    .def = 0,  /* Default to UYVY */
};
...
static int rs300_init_controls(struct rs300 *rs300)
{
    ...
    ret = v4l2_ctrl_handler_init(ctrl_hdlr, 27);  /* 22 controls + HBLANK + VBLANK + EXPOSURE + ANALOGUE_GAIN */
    if (ret) {
    ...
    pixel_rate = rs300_get_pixel_rate(MEDIA_BUS_FMT_YUYV8_1X16);
...
static int __maybe_unused rs300_get_device_name(struct rs300 *rs300)
{
    struct i2c_client *client = v4l2_get_subdevdata(&rs300->sd);
    u8 cmd_buffer[18];
    u8 status_buffer[1];
...
static int rs300_probe(struct i2c_client *client)
{
	...
	/* Parse Rockchip module identification from device tree (non-fatal) */
    ...
    */
	...
	ret = rs300_set_yuv_format(rs300, 0); /* 0 = UYVY format, 1=VYUY format, 2 = YUYV format, 3=YVYU format */
	if (ret) {
	...
	if (rs300->sd.ctrl_handler) {
		dev_dbg(dev, "Subdevice has control handler initialized successfully\n");
	} else {
...
static void rs300_remove(struct i2c_client *client)
{
	...
	/* Stop streaming before teardown to avoid dangling MIPI state */
	if (rs300->streaming)
		rs300_set_stream(sd, 0);
...
static struct i2c_driver rs300_i2c_driver = {
	...
	/*
	 * Use .probe_new (single-arg) — the legacy .probe field requires
	 * (struct i2c_client *, const struct i2c_device_id *) and produces
	 * an -Werror=incompatible-pointer-types on stock Radxa BSP kernels
	 * (silenced only on the -999 custom build by an extra KBUILD_CFLAGS
	 * override). Both .probe and .probe_new are dispatched by i2c-core in
	 * kernel 6.1; .probe_new has been the idiomatic form since 5.x.
	 */
...
MODULE_AUTHOR("Infisense <support@infisense.com>");
MODULE_DESCRIPTION("Infisense RS300 thermal camera sensor driver");
